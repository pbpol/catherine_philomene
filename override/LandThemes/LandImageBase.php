...
require_once (_PS_ROOT_DIR_.'/override/LandThemes/phpthumb/PhpThumb.inc.php');
require_once (_PS_ROOT_DIR_.'/override/LandThemes/phpthumb/ThumbLib.inc.php');
class LandImageBase
{
    /**
    ...
    public static function makeDir($path)
    {
        ...
        $tmppath = _PS_ROOT_DIR_."/upload/thumbs/";
    ...
    public static function renderThumbProduct($path,$rewrite="", $width = 100, $height = 100,$type = '', $title = '', $isThumb = true, $image_quanlity = 100, $returnPath = false, $class = "")
    {
        if (!preg_match("/.jpg|.png|.gif/", strtolower($path))) return '&nbsp;';
        if ($isThumb) {
            if (empty($image_quanlity)) {
            ...
            $imagSource = _PS_ROOT_DIR_."/img/p/". $path;
            if (file_exists($imagSource)) {
                if($type!=''){
            ...
            if (file_exists($imagSource)) {
                ...
                if (!file_exists($thumbPath)) {
                    ...
                    if(!preg_match("/.gif/", strtolower($path))){
                        ...
                        LandImageBase::makeDir($thumbPath);
                    }
                    $thumb->save($thumbPath);
                }
                $path = $thumbPath;
