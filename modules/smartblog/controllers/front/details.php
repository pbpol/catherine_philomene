...
if (!class_exists( 'LandImageBase' )) {     
    require_once (_PS_ROOT_DIR_.'/override/LandThemes/LandImageBase.php');
}
	...
	public function initContent()
	{
           ...
           $title_category = $BlogCategory->getNameCategory($id_cate);
            if (file_exists(_PS_MODULE_DIR_.'smartblog/images/' . Tools::getValue('id_post') . '.jpg') )
                {
                    $image = Tools::getValue('id_post') . '.jpg';
                   $post_img =  Tools::getValue('id_post');
                   $pathImg = "smartblog/images/".$image;
                  $width=960;$height=540;
                  $thumb_image = LandImageBase::renderThumb($pathImg,$width,$height);
                }else{
                    $post_img = 'no';
                }
