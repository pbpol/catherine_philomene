...
class labslideshow extends Module
{
	...
	public function install()
	{
		...
		)
		{
			...
			$res &= Configuration::updateValue('LAB_PAUSE', '5000');
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 2; ++$i)
		{
			...
			{
				$slide->title[$language['id_lang']] = 'style women';
				$slide->description[$language['id_lang']] = '<p>collection 2015</p>';
				$slide->legend[$language['id_lang']] = 'big sale';
				$slide->url[$language['id_lang']] = '#';
				$slide->image[$language['id_lang']] = 'labSlider-'.$i.'.jpg';
				$slide->margin = $margin[$i];
			}
			$slide->add();
		}
	}
...
//echo "<pre>".print_r($_POST,1);die;
	...
	public function hookdisplayHeader($params)
	{
		...
		$this->context->controller->addCSS($this->_path.'css/demo.css');
		$this->context->controller->addCSS($this->_path.'css/flexslider.css');
		$this->context->controller->addCSS($this->_path.'css/normalize.css');
	...
	//	$this->context->controller->addJS($this->_path.'js/jquery.nivo.slider.pack.js');
		//$this->context->controller->addJS($this->_path.'js/demo.js');
...
/* 		$slider = array(
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`,hss.`margin`
		...
		$fields_form = array(
			'form' => array(
				...
				'input' => array(
					...
					array(
						'type' => 'switch',
						'label' => $this->l('Enabled Auto slides'),
						'name' => 'LAB_TITLE',
						'values' => array(
							array(
								'id' => 'active_on',
