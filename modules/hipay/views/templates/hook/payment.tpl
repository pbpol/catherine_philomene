...
<p class="payment_module hipay_payment_module">
	{if $cart->getOrderTotal() < 2}
		<a href="">
			<img src="{$this_path|escape:'htmlall':'UTF-8'}views/img/payment_button/{$logo_suffix|escape:'htmlall':'UTF-8'}.png" alt="{l s='Pay with Hipay' mod='hipay'}" />
			{l s='Minimum amount required in order to pay with Hipay:' mod='hipay'} {convertPrice price=2}
		</a>
	{else}
	<a href="{$redirection_url|escape:'htmlall':'UTF-8'}" title="{l s='Pay with Hipay' mod='hipay'}">
		<img src="{$this_path|escape:'htmlall':'UTF-8'}views/img/payment_button/{$logo_suffix|escape:'htmlall':'UTF-8'}.png" alt="{l s='Pay with Hipay' mod='hipay'}" />
