...
*/
    ...
    public function hasCartLocker($cartId)
    {
        ...
        $sql = 'SELECT COUNT(*) FROM `%setransactions_cart_locker` WHERE `id_cart` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $cartId);
    ...
    public function createCartLocker($cartId)
    {
        $cartId = intval($cartId);
        if (empty($cartId)) {
            return false;
