...
class labhomefeatured extends Module
{
	protected static $cache_products;

	public function __construct()
	{
		$this->name = 'labhomefeatured';
	...
	public function getContent()
	{
		$output = '';
		$errors = array();
		if (Tools::isSubmit('submitlabhomefeatured'))
		{
			...
			else
			{
				...
				Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('labhomefeatured.tpl'));
	...
	{
		if (!isset(labhomefeatured::$cache_products))
		{
			$category = new Category((int)Configuration::get('HOME_FEATURED_CAT'), (int)Context::getContext()->language->id);
			$nb = (int)Configuration::get('HOME_FEATURED_NBR');
			if (Configuration::get('HOME_FEATURED_RANDOMIZE'))
				labhomefeatured::$cache_products = $category->getProducts((int)Context::getContext()->language->id, 1, ($nb ? $nb : 8), null, null, false, true, true, ($nb ? $nb : 8));
			else
				labhomefeatured::$cache_products = $category->getProducts((int)Context::getContext()->language->id, 1, ($nb ? $nb : 8), 'position');
		}

		if (labhomefeatured::$cache_products === false || empty(labhomefeatured::$cache_products))
			return false;
	}
	...
	{
		if (!$this->isCached('labhomefeatured.tpl', $this->getCacheId()))
		{
			$this->_cacheProducts();
			$this->smarty->assign(
				array(
					'products' => labhomefeatured::$cache_products,
		...
		return $this->display(__FILE__, 'labhomefeatured.tpl', $this->getCacheId());
	}
	...
	{
		parent::_clearCache('labhomefeatured.tpl');
	}
	...
	public function renderForm()
	{
		...
		$helper->submit_action = 'submitlabhomefeatured';
