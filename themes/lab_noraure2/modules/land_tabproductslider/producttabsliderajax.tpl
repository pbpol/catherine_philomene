<div class="product-tabs-slider lablistproducts laberthemes">
	<div class="lab_tabs">
		<ul class="tabs" data-ajaxurl="{$base_dir}modules/land_tabproductslider/producttabslider-ajax.php">
			{$count=1}
			{foreach from=$productTabslider item=productTab name=posTabProduct}
				<li class="{if $smarty.foreach.posTabProduct.first}first_item{elseif $smarty.foreach.posTabProduct.last}last_item{else}{/if} {if $productTab.default==$defaultTab} active {/if}" rel="tab_{$productTab.id}"  >
					{$productTab.name}
				</li>
				{$count= $count+1}
			{/foreach}
	...
	<div class="producttab row">
		<div class="tab_container fx-fadeInUp">
			<div id="tab_{$defaultTab}" class="tab_content">
				<div class="productTabContent productTabContent_{$defaultTab} product_list productContent">
					{foreach from=$productData item=product name=posProducts}
								...
								<div class="lab-cart">
									...
									{if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && $product.customizable != 2 && !$PS_CATALOG_MODE}
										{if ($product.allow_oosp || $product.quantity > 0)}
											...
											{/if}
										{else}
											<a class="lab-cart-i button ajax_add_to_cart_button btn btn-default disabled">
														<!--<span>{l s='Add to cart' mod='land_tabproductslider'}</span>-->
														<span>Ajouter au panier</span>
													</a>
										{/if}
		...
		<script>
			$(document).ready(function() {
				var owl = $(".productTabContent_{$defaultTab}");
				owl.owlCarousel({
					autoPlay : false,
					items :4,
					itemsDesktop : [1200,3],
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		jQuery("ul.tabs li").click(function(){
			var idTab = jQuery(this).attr("rel").replace("tab_","");
			var checkExist = jQuery(".productTabContent_"+idTab).html();
			if(jQuery.trim(checkExist).length == 0 || checkExist == undefined){
				jQuery(".product-tabs-slider.lablistproducts .sliderLoadingTab").remove();
				jQuery(".product-tabs-slider.lablistproducts").append('<span class="sliderLoadingTab">Loading...</span>');
				...
				jQuery.ajax({
					...
					success:function(data){
						...
						jQuery(".product-tabs-slider.product-tabs-slider .sliderLoadingTab").remove();
						jQuery(".product-tabs-slider").append(results);
						return false;
					}
				});
			}
