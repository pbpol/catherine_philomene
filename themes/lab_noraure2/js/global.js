...
function display(view)
{
	if (view == 'list')
	{
		...
		$('.product_list > li').removeClass('col-lg-4 col-md-4 col-sm-6 col-xs-12').addClass('col-xs-12');
		$('.product_list > li').each(function(index, element) {
			html = '';
			html = '<div class="product-container item wow fadeInUp" data-wow-delay="200ms"><div class="row">';
				html += '<div class="left-block topItem col-mobile col-xs-6 col-sm-5  col-md-4">' + $(element).find('.left-block').html() + '</div>';
				html += '<div class="right-block col-mobile col-xs-6 col-sm-7 col-md-8">';
					html += '<div class="product-flags">'+ $(element).find('.product-flags').html() + '</div>';
					html += '<h5 itemprop="name" class="h5product-name">'+ $(element).find('h5').html() + '</h5>';
					var rating = $(element).find('.comments_note').html(); // check : rating
					if (rating != null) {
						html += '<div itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating" class="comments_note">'+ rating + '</div>';
					}
					var price = $(element).find('.lab-price').html(); // check : catalog mode is enabled
					if (price != null) {
						html += '<div class="lab-price">'+ price + '</div>';
					}
	...
	else
	{
		...
		$('.product_list > li').removeClass('col-xs-12').addClass('col-lg-4 col-md-4 col-sm-6 col-xs-12');
		$('.product_list > li').each(function(index, element) {
		html = '';
		html += '<div class="product-container item wow fadeInUp" data-wow-delay="200ms">';
			html += '<div class="left-block topItem">' + $(element).find('.left-block').html() + '</div>';
			html += '<div class="bottomItem">';
				html += '<div class="product-flags">'+ $(element).find('.product-flags').html() + '</div>';
				html += '<h5 itemprop="name" class="h5product-name">'+ $(element).find('h5').html() + '</h5>';
				var price = $(element).find('.lab-price').html(); // check : catalog mode is enabled
...
$(window).bind('scroll', function() {
     if ($(window).scrollTop() > 120) {
         $('.labheader,.ac_results').addClass('fixed');
         $('.top-link').addClass('hide');
     }
     else {
     	 $('.top-link').removeClass('hide');
         $('.labheader,.ac_results').removeClass('fixed');
     }
});

function checkWidth() {
   if ($(window).width() < 980) {
    $('#header_block_top').addClass('header_hover');
} else {
    $('#header_block_top').removeClass('header_hover');
}
}
